...
exports.PathInference = void 0;
exports.fragmentize = fragmentize;
exports.isTemplated = isTemplated;
exports.looksLikeAVariable = looksLikeAVariable;
exports.reservedPatterns = void 0;
...
class PathInference {
  ...
  static async fromSpecAndInteractions(spec, interactions) {
    ...
    for (const {
      pathPattern
    } of (0, _queries.specToPaths)(spec)) {
  ...
  getInferedPattern(url) {
    const builtUrl = [];
    let currentNode = this.root;
    for (const part of fragmentize(url)) {
      const siblingConstantKeys = [];
      ...
      const parentPart = builtUrl[builtUrl.length - 1] ?? null;
      const isFirst = parentPart === null;
      const parentIsVariable = parentPart !== null && isTemplated(parentPart);
      const parentIsResource = parentPart !== null && reservedPatterns.some(r => r.test(parentPart));
      const hasAnyConstantSibilings = siblingConstantKeys.length > 0;
      const isVariable = looksLikeAVariable(part);
      const uniqueUrlsStartingWithCurrent = [...this.observedUrls.values()].filter(url => {
        ...
        for (let i = 0; i < builtUrl.length; i++) {
          if (!isTemplated(builtUrl[i]) && builtUrl[i] !== urlFragments[i]) return false;
        }
        return true;
      });
      ...
      if (isFirst || parentIsVariable || parentIsResource) {
        ...
        if (isVariable || uniqueUrlsStartingWithCurrent.length >= COLLAPSE_CONSTANTS_N && !hasAnyConstantSibilings) {
          const variableName = _pluralize.default.singular(parentPart);
...
exports.PathInference = PathInference;
...
const reservedPatterns = exports.reservedPatterns = [/^api$/, /^v[0-9]+$/, /^[0-9]+\.[0-9]+$/, /^20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/];
function looksLikeAVariable(stringValue) {
  if (reservedPatterns.some(pattern => pattern.test(stringValue))) return false;
  if (stringValue) if (!isNaN(Number(stringValue)))
    // any number is a variable
    return true;
  // any uuid is a variable
  if (/^[0-9A-F]{8}-[0-9A-F]{4}-[4][0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i.test(stringValue) || /^[0-9A-F]{8}-[0-9A-F]{4}-[5][0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i.test(stringValue)) return true;
  return false;
}