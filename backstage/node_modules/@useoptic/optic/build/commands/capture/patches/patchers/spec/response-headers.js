...
exports.generateResponseHeaderPatches = generateResponseHeaderPatches;
var _patches = require("./patches");
var _jsonPointerHelpers = require("@useoptic/json-pointer-helpers");
var _types = require("./types");
async function* generateResponseHeaderPatches(interaction, operation) {
  ...
  const responsePath = _jsonPointerHelpers.jsonPointerHelpers.compile(['paths', operation.pathPattern, operation.method, 'responses', interaction.response.statusCode]);
  ...
  for (const [key, value] of Object.entries(responseObject.headers ?? {})) {
    ...
    if (!interaction.response.headers.find(h => h.name === key)) {
      const headerPath = _jsonPointerHelpers.jsonPointerHelpers.append(responsePath, 'headers', key);
      yield {
        ...
        diff: {
          kind: _types.OperationDiffResultKind.MissingRequiredResponseHeader,
          statusCode: interaction.response.statusCode,
          name: key
        },
        path: headerPath,
        groupedOperations: [{
          op: 'replace',
          path: _jsonPointerHelpers.jsonPointerHelpers.append(headerPath, 'required'),
          value: false
        }],
        interaction
      };
