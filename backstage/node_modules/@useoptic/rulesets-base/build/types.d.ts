import { FactVariant, FlatOpenAPIV3, FlatOpenAPIV3_1, OpenApiKind, OpenAPIV3 } from '@useoptic/openapi-utilities';
export type OpenAPIDocument = OpenAPIV3.Document | FlatOpenAPIV3.Document | FlatOpenAPIV3_1.Document;
...
export type AssertionType = 'specification' | 'operation' | 'query-parameter' | 'path-parameter' | 'header-parameter' | 'cookie-parameter' | 'response' | 'response-header' | 'request-body' | 'response-body' | 'property' | 'schema';
...
type MatchesFn = (structure: any, options?: {
    strict?: boolean;
    errorMessage?: string;
}) => void;
type MatchesOneOfFn = (structures: any[], options?: {
    strict?: boolean;
    errorMessage?: string;
}) => void;
...
type ResponseAssertionHelpers = {
    ...
    hasResponseHeaderMatching: (name: string, structure: any, options?: {
        strict?: boolean;
    }) => void;
};
...
export type Assertion<T extends AssertionType> = (value: AssertionTypeToValue[T]) => void;
export type RegisterAssertion<T extends AssertionType> = (...args: [condition: string, assertion: Assertion<T>] | [assertion: Assertion<T>]) => void;
export type ChangedAssertion<T extends AssertionType> = (before: AssertionTypeToValue[T], after: AssertionTypeToValue[T]) => void;
export type RegisterChangedAssertion<T extends AssertionType> = (...args: [condition: string, assertion: ChangedAssertion<T>] | [assertion: ChangedAssertion<T>]) => void;
